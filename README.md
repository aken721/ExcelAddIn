...
将插件的 111 个 Excel 操作工具封装为**通用型 MCP Skill**，可在 openclaw、hermes、workbuddy、qclaw、trae work 等**独立 Agent** 中安装调用，无需安装 Excel。

- **协议**：MCP (Model Context Protocol) 2024-11-05，stdio 传输
- **引擎**：ClosedXML（无需安装 Excel）
- **功能**：工作簿/工作表/单元格/区域/格式/图表/分析/数据库/API/邮件/文件/二维码/发票/正则/目录/定时/PDF 等 111 个工具
- **性能**：MCP 模式下单进程持久运行，10次建表仅 4ms；复合工具一次调用完成多步操作

**安装方式**：

1. **下载预编译安装包**（推荐）：下载 `table-magic-skill.zip`，解压到 Agent 的技能安装目录即可，Agent 会自动通过 `skill.md` 和 `mcp-config.json` 发现并注册本技能；或者将 `table-magic-skill.zip` 拖入 Agent 对话框，并发出安装本技能的指令后，Agent 自动安装并注册本技能
2. **从源码构建**：`git clone` 本仓库，运行 `pack-skill.ps1` 脚本自动构建并生成 `table-magic-skill.zip`，然后采用“1.”中所述方法安装技能

详见 [TableMagic.Cli/README.md](./TableMagic.Cli/README.md)
