...
### 说明：
本项目为个人维护的开源项目，完全免费可放心使用。如果您喜欢该项目，请多点Star给予鼓励。
...
5. **Skills扩展架构重构**（重要更新）：
   ...
   **新旧架构对比**：
   ...
   | 对比项 | 原有模式 | Skills扩展架构 |
   |--------|----------|---------------|
   | 工具定义位置 | 分散在多个文件中 | 每个Skill类集中定义 |
   | 工具执行逻辑 | 与UI代码耦合 | 独立的SkillManager管理 |
   | 新增工具 | 需修改多处代码 | 只需添加新Skill类 |
   | 工具分组 | 硬编码 | 动态配置 |
   | 参数映射 | 固定参数名 | 支持参数别名 |
   | 错误处理 | 分散处理 | 统一SkillResult返回 |
   | 可扩展性 | 较差 | 优秀 |

   **当前支持的技能模块**：
   - ExcelBaseSkill：基础操作（获取工作表名称等）
   - ExcelWorkbookSkill：工作簿操作（创建、打开、保存、关闭）
   - ExcelSheetSkill：工作表操作（创建、删除、重命名、复制、移动、冻结）
   - ExcelCellSkill：单元格操作（读写值、公式）
   - ExcelRangeSkill：区域操作（批量读写、复制、清除）
   - ExcelFormatSkill：格式设置（字体、颜色、边框、合并、对齐）
   - ExcelRowColSkill：行列操作（插入、删除、隐藏、调整大小）
   - ExcelChartSkill：图表操作（创建图表）
   - ExcelPivotSkill：数据透视表
   - ExcelAnalysisSkill：数据分析（统计、查找替换）
   - ExcelFinanceSkill：财务分析



**2026-01-30** 主要更新内容： 1.原“deepseek”改为“AI对话”； 2.AI对话的设置中增加等待模型响应时间的自定义配置项； 3.AI对话窗口增加“优先使用提示工程模式”开关，便于使用本地模型时可设置为强制不使用function calling模式，减少等待时间

**2025-12-25**
v2.4.7.0 主要更新内容：

1. 图表增强中增加动态图表生成，可生成html交互式动态图表和gif动图图表（目前暂只支持条形动态图和柱状动态图）
2. 图表增强中数据区域支持从Excel表格中获取选中区域
3. 批量生成word文档功能中增加生成范围可配置选项，可选择生成excel表中的全部行数据或选择行数据
4. 批读文件名中完善了"文件/目录"和"包含子目录"选项的读取逻辑，即时已生成批读文件名的表格后，改变选项会自动重新按照新逻辑刷新_rename表内容
5. 解决了部分已知问题
6. 优化了部分UI界面
...
1. 修复批量生成word文档时，模板中多个相同的图片占位符只被替换一次的问题。
2. 弃用mp3批量改名，因与文件批量重命名功能重复。
...
**（注：MCP工具模式已在v2.5.0.0版本重构为Skills扩展架构，详见Skills使用指南.md）**
...
3.  二维码识别，可识别图片中的二维码或通过电脑摄像头扫码，将二维码中记录数据提取到Excel表格中；
4.  电子发票批量导入，支持xml格式数电票和pdf、图片格式电子发票，批量提取发票中信息到Excel表格中，方便发票管理；
5.  文档批量生成，可将word模板中的占位符批量替换为Excel中记载内容，一行数据生成一个Word文档，支持图片读取插入文档；
6.  Email群发，支持针对不同收件人自动群发不同的附件；
7.  定时任务管理，windows任务计划风格的定时任务设置，支持任何频率的cmd命令执行、vba代码执行、bat脚本运行、python代码运行、exe程序运行等；
8.  批量修改文件名/目录名、批量删除或移动文件/目录：对指定目录下的文件/子目录，读取进入Excel表格，“A列”为源文件/目录路径，“B列”为源文件名/目录名，“C列”为新文件名/目录名。只需通过Excel表的查找、替换、分列等功能，编辑"C列”后，即可高度定制化地批量修改文件名/目录名，或批量删除（移动）文件名/目录名；
9.  Excel转为PDF功能，可选：当前表转PDF、工作簿所有表转为单一PDF、工作簿所有表转为多个PDF等三种转换方式，并可设定页面大小、页面方向和页面缩放比例等；
10.  播放音乐：在操作excel表的同时播放指定文件夹内的音乐，目前支持“mp3、wav、flac、aiff、wma、aac、g711、mp4”等音乐格式；
11. ~~接入deepseek，支持deepseek通过内置mcp操控Excel，可以用自然语言实现Excel的读、写数据和其他复杂任务。~~ **（注：MCP工具模式已在v2.5.0.0版本重构为Skills扩展架构，详见Skills使用指南.md）**
